...
(function (global) {
   ...
   *  distincte du quadrillage fin des bandeaux de catégorie (buildGridBorder ci-dessus, thin).
   *  Sert de "cadre" à gauche de "Rachat — cash reçu" et à droite de "Pénalité de sortie" : jamais
   *  de trait entre les lignes de fonds elles-mêmes, ni entre ces 2 colonnes. Un fichier client
   *  réel a confirmé ce constat (design vérifié cellule par cellule par le conseiller) : les
   *  lignes de fonds n'ont AUCUNE bordure haute/basse, seule une bordure verticale medium encadre
   *  le bloc des 2 colonnes en continu.
   ...
   *  présente (colonnes cachées entre TOTAL et cashCol par ex.), `adjacentCol`, s'il est fourni,
   *  fait reprendre la bordure de LA colonne réellement adjacente plutôt qu'une colonne "contrat"
   *  quelconque — garantissant une continuité visuelle exacte avec ce qui existe déjà juste à côté. */
  function buildVerticalFrameSide(ws, sampleRow, adjacentCol) {
    const fallback = { style: "medium", color: { argb: "FFB8A490" } };
    if (!sampleRow) return fallback;
    if (adjacentCol) {
      const adjBorder = ws.getCell(sampleRow, adjacentCol).border || {};
      const adjSide = adjBorder.right || adjBorder.left;
      if (adjSide && adjSide.style) return JSON.parse(JSON.stringify(adjSide));
    }
   ...
   *  chacune optionnelle — et 2 phrases pour les pénalités actives datées) — calculé à partir de
   *  la longueur RÉELLE du texte de la base (connue à la génération), pas devinée : un texte
   *  source inhabituellement long (ça arrive) donne une ligne plus haute plutôt qu'un texte
   *  coupé. */
  function estimatePenaltyLines(kind, rawLen, dureeLen, conservationLen, charsPerLine) {
    ...
    if (kind === "ferme") {
      ...
      if (conservationLen) lines += linesFor(conservationLen);
      return lines;
    }
    if (kind === "manuel") {
  ...
  function classifyRows(ws, headerRow, totalCol) {
    ...
    const rowToCategoryFill = {};
    let currentCategory = null;
    let currentCategoryFill = null;
    ...
    for (let r = headerRow + 2; r <= lastRow; r++) {
      ...
      if (isSolidBold(a, bCell)) {
        ...
        rowToCategoryFill[r] = currentCategoryFill;
      }
    }
    return { categoryRows, fundRows, rowToCategory, rowToCategoryFill };
  }
  ...
  function buildExitSheet(workbook, srcWs, headerRow, calendar, calLastRow, penLastRow, fundsByIsin, refDateIso) {
    ...
    const { fundRows, rowToCategory, rowToCategoryFill } = classifyRows(srcWs, headerRow, totalCol);
    ...
    fundRows.forEach((srcRow) => {
      ...
      const categoryFill = rowToCategoryFill[srcRow] || null;
      ...
      holdingByOwner(srcWs, srcRow, totalCol, ownerLabels).forEach(([owner, amount]) => {
        ...
        if (!isHeld) return;
        selected.push({ isin, nom, category, categoryFill, owner, amount, needsDate, penalite: fund.penalite });
      });
    });
    ...
    function writeHeaderRow(atRow) {
      headers.forEach((label, i) => {
        ...
        // Aucune bordure sur la ligne d'en-tête elle-même (ni le quadrillage fin des catégories,
        // ni le cadre épais hérité de headerStyle) : demandé explicitement, la ligne "Fonds | ISIN
        // | ..." doit rester sans trait, seul son fond coloré la distingue.
        cell.border = {};
      });
    ...
    function writeSimBanner(atRow) {
      setF(ws, `A${atRow}`, `"Si Date de Retrait Exécuté : "&${frDateExpr(`${RD}$1`)}`);
      for (let c = 1; c <= headers.length; c++) {
        const cell = ws.getCell(atRow, c);
        cell.style = JSON.parse(JSON.stringify(headerStyle));
        // headerStyle porte le cadre épais ("medium") de l'en-tête de Consolidation : sans
    ...
    ownerOrder.forEach((ownerKey, ownerIdx) => {
      ...
      byOwner.get(ownerKey).forEach((item) => {
        if (categoryStyle && item.category !== currentCategory) {
          ...
          // modèle unique emprunté à la 1re catégorie trouvée) : Consolidation utilise 2 teintes
          // de beige distinctes selon le niveau d'imbrication, à reproduire fidèlement ici.
          const bandFill = item.categoryFill || (categoryStyle.fill ? JSON.parse(JSON.stringify(categoryStyle.fill)) : { type: "pattern", pattern: "none" });
          for (let c = 1; c <= headers.length; c++) {
            ws.getCell(r, c).style = { ...JSON.parse(JSON.stringify(categoryStyle)), fill: JSON.parse(JSON.stringify(bandFill)) };
        ...
        // Même remarque que pour ${minifsField} plus bas : MINIFS renvoie 0 (jamais une erreur)
        // quand ce fonds a bien des échéances "Rachat" (${L}>0) mais qu'AUCUNE n'est encore à
        // venir par rapport à la date de référence (toutes déjà passées) — sans la vérification
        // "=0" ci-dessous, Excel afficherait ce 0 comme une date, "00/01/1900", au lieu de laisser
        // la cellule vide.
        {
        ...
        setF(ws, `I${r}`,
          ...
          // Le fonds a bien des échéances "Rachat" connues (${L}>0) mais aucune à partir de la
          // date de référence choisie (${M} vide) : le calendrier officiel ne couvre pas assez
          // loin dans le temps pour répondre — à signaler explicitement plutôt que de laisser les
          // colonnes de rachat silencieusement vides.
          `AND(${L}${r}>0,${M}${r}=""),"Calendrier de rachat non disponible pour cette date : le calendrier officiel doit être mis à jour.",` +
        ...
        const lines = estimatePenaltyLines(penInfo.kind, (penInfo.raw || "").length, (penInfo.dureeVie || "").length, (penInfo.conservation || "").length, CHARS_PER_LINE);
  ...
  function addConsolidationColumns(srcWs, headerRow, totalCol, exitSheetName, firstDataRow, lastDataRow, fundsByIsin, isSimulating) {
    ...
    // distinctes selon le niveau d'imbrication (ex. "Private Equity" en beige foncé, une
    // sous-catégorie en beige clair) — on reprend donc le fond de CHAQUE ligne de catégorie
    // individuellement (pas un seul modèle emprunté à la 1re catégorie trouvée et réappliqué
    // partout), pour prolonger fidèlement ces 2 teintes sur les 2 nouvelles colonnes.
    ...
    // du tableau (catégories ET fonds, y compris les éventuelles lignes vides entre 2 fonds d'une
    ...
    // jamais du quadrillage fin de gridBorder qui, lui, sert uniquement aux catégories.
    ...
    const sampleRow = fundRows[0] !== undefined ? fundRows[0] : categoryRows[0];
    const leftFrameSide = buildVerticalFrameSide(srcWs, sampleRow, cashCol - 1);
    const rightFrameSide = buildVerticalFrameSide(srcWs, sampleRow);
    ...
    if (categoryRows.length || fundRows.length) {
      const categorySet = new Set(categoryRows);
      const bodyFirstRow = Math.min(...categoryRows, ...fundRows);
      const bodyLastRow = Math.max(...categoryRows, ...fundRows);
      for (let r = bodyFirstRow; r <= bodyLastRow; r++) {
        if (categorySet.has(r)) {
          ...
          srcWs.getCell(r, cashCol).style = {
            ...
            border: { top: gridBorder.top, bottom: gridBorder.top, left: leftFrameSide },
          };
          srcWs.getCell(r, penCol).style = {
            fill: JSON.parse(JSON.stringify(rowFill)),
            border: { top: gridBorder.top, bottom: gridBorder.top, right: rightFrameSide },
          };
        } else {
          srcWs.getCell(r, cashCol).style = { fill: { type: "pattern", pattern: "none" }, border: { left: leftFrameSide } };
          srcWs.getCell(r, penCol).style = { fill: { type: "pattern", pattern: "none" }, border: { right: rightFrameSide } };
        }
      }
    }
    ...
    fundRows.forEach((r) => {
      ...
      // inhabituellement long, pour ne jamais couper le texte de pénalité — quitte à rendre cette
      // ligne précise plus haute que ses voisines (le texte lisible passe avant la régularité
      // visuelle du tableau).
      const fund = fundsByIsin.get(isin);
      const penInfo = (fund && fund.penalite) || { kind: "inconnue", raw: null };
      const lines = estimatePenaltyLines(penInfo.kind, (penInfo.raw || "").length, (penInfo.dureeVie || "").length, (penInfo.conservation || "").length, CHARS_PER_LINE);
      const neededHeight = heightForLines(lines, baseFontSize);
    ...
    // une note en italique sous le tableau) — sur les colonnes d'origine uniquement, et SANS
    // cadre vertical à cette ligne précise (vérifié sur un fichier réel : ni gauche ni droite,
    // seulement le trait horizontal). Sans la reprendre aussi sur les 2 nouvelles colonnes, la
    // ligne de fermeture du tableau s'arrête net juste avant elles ; mais y ajouter le cadre
    // vertical ferait paraître le tableau "descendre" d'une ligne de trop, au-delà de sa vraie
    // dernière ligne. On ne recopie donc que la PRÉSENCE d'une bordure de clôture à cet endroit
    // (pour savoir s'il y en a une) et son style fin (comme les bandeaux de catégorie) — jamais le
    // cadre épais d'origine, ni de cadre vertical.
    if (categoryRows.length || fundRows.length) {
      ...
      if (hasClosingBorder) {
        srcWs.getCell(closingRow, cashCol).style = { fill: { type: "pattern", pattern: "none" }, border: { top: gridBorder.top } };
        srcWs.getCell(closingRow, penCol).style = { fill: { type: "pattern", pattern: "none" }, border: { top: gridBorder.top } };
      }
    }
  ...
  /** Formule Excel "JJ/MM/AAAA" pour la date renvoyée par `dateExpr`, SANS utiliser
  ...
  function frDateExpr(dateExpr) {
    return `TEXT(DAY(${dateExpr}),"00")&"/"&TEXT(MONTH(${dateExpr}),"00")&"/"&YEAR(${dateExpr})`;
  }
  ...
  async function buildExitCalendarWorkbook(arrayBuffer, fundsData, refDateIso) {
    ...
    const totalCol = findTotalColumn(srcWs, headerRow);
    addConsolidationColumns(srcWs, headerRow, totalCol, exitWs.name, firstDataRow, lastDataRow, fundsByIsin, isSimulating);
