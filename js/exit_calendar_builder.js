...
(function (global) {
  ...
  function applyPrintSetup(ws, lastRow, lastVisibleCol) {
    // Marge de 2 lignes/colonnes entre le contenu et le contour de la zone d'impression. Pour
    ...
    // au milieu de colonnes masquées au lieu de se terminer 2 colonnes après elles.
    const lastHelperCol = HELPER_FIRST_COL + HELPER_NAMES.length - 1;
    const lastCol = Math.max(lastVisibleCol, lastHelperCol);
    ws.pageSetup.printArea = `A1:${colLetter(lastCol + 2)}${lastRow + 2}`;
